...
def makesingletestfile_nomanual(funcname):
    ...
    for o in codelines:
        if 'int i' in o:
            ...
            for v in varlist:
                file.write(v+"=randfl_uniReal(range);\n")
        elif 'FPSynx10' in o:
...
def makesingletestfile_manual(funcname):
    IA.IAprogram(funcname)
    filename=funcname+'/'+"timetestsingle.c"
    file=open(filename,'w')
    headfile=open('header.txt','r')
...
def makeprobtestfile_nomanual(funcname):
    IA.IAprogram(funcname)
...
def makeprobtestfile_manual(funcname):
    IA.IAprogram(funcname)
    filename=funcname+'/'+"probtest.c"
    file=open(filename,'w')
    headfile=open('header.txt','r')
...
def makeruntestfile_nomanual(funcname):
...
def makeruntestfile_manual(funcname):
    MPFRGen.Exactprogram(funcname)
    filename=funcname+'/'+"runtest.c"
    file=open(filename,'w')
    headfile=open('header.txt','r')
...
for file in manuallist:
    makesingletestfile_manual(file)
    makeprobtestfile_manual(file)
    makeruntestfile_manual(file)
for file in nomanuallist:
    makesingletestfile_nomanual(file)
    makeprobtestfile_nomanual(file)    
    makeruntestfile_nomanual(file)