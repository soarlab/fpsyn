...
def countopsold(exp,prefix='c'):
    inter=get_intermediate(exp,prefix)
    return inter, len(inter)

def countops(exp,prefix='c'):
    mexp = collect_const(exp)
    if mexp.func != Mul:
        return countopsold(exp,prefix)
    if (mexp == exp):
        return countopsold(exp,prefix)       
    exp1 = mexp.args[1]
    cons = mexp.args[0]
    asgo, numopo = countopsold(exp,prefix)
    asg1, numop1 = countopsold(exp1,prefix)
    if numopo<numop1+1:
        return countopsold(exp,prefix)
    s,v = asg1[-1]
    sfinal = symbols(prefix+str(numop1))
    exfinal = cons*s
    asg1.append((sfinal,exfinal))
    return asg1, numop1+1
