...
def subsexp_adjustcoeff_final(exp,subsmul,subsadd,thrhld):
    ...
    print(subs1)
    adjust = adjust_coeff_deep(subs1,thrhld)
    #adjust = simplify(adjust)
    print(adjust)
    subs2,numop2=subsexp_noex(adjust,subsmul,subsadd)
    #subs2 = simplify(subs2)
    if numop2<=numop1:
        return subs1, adjust, subs2, subs2, numop2
    else:
        return subs1, adjust, subs2, subs1, numop1
...
def combineasg(exp1, exp2, asg1,asg2,inter,n,d):
    sl=signlist(inter)
    absop1=absops(exp1,inter,sl)
    absop2=absops(exp2,inter,sl)
    absop=[]
    for e in absop1:
        absop.append(e)
    for e in absop2:
        if e not in absop:
            absop.append(e)        
    t,ex=inter[len(inter)-1]
    denabs=(d,'fabs('+str(d)+')')
    numabs=(n,'fabs('+str(n)+')')
    if numabs not in absop:
        absop.append(numabs)
    if denabs not in absop:
        absop.append(denabs)
    ...
    for e in asg2:
        asg.append(e)
    asg=editoplist(asg,'temp')
    numerrvar,ex=asg[len(asg1)-1]
    denerrvar,ex=asg[len(asg)-1]
    out=[]
    for e in absop:
        out.append(e)
    for e in asg:
        out.append(e)            
    return out, len(out), numerrvar, denerrvar
...
def countops_partition(exp,assign,numpar,prefix='tem'):
    par=partition(exp,assign,numpar)
    asglist=[]
    parstring = ''
    count=0
    ...
    asglist=editoplist(asglist,prefix)   
    numop=len(asglist)
...
def tabusearch(exp,numpar,numsearch,listsize,cyclelen,secondcyclelen,file,prefix='tem'):
    if exp.func!=Add:
    ...
    print(result)
    bestexp, assign, bestnumop = countops_partition(exp,result,numpar,prefix)
    print('result:')
    print(bestexp)
    prtlist(assign)
    print(bestnumop) 
...
def FPSynthesisDiv(num,den,funcname):
    epval=2**(-53)
    thrhld=3*epval
    div=Function('div')
    ...
    inter=get_intermediate(orgexp,'t')
    (a,ex)=inter[len(inter)-1]
    numer=ex.args[0]
    denom=ex.args[1]
    inter=inter[0:len(inter)-1]
    numinter=len(inter)    
    ...
    bestexp, tabuassign, bestnumop=tabusearch(errexp,numpar,numsearch,listsize,cyclelen,secondcyclelen,file,'num')

    asg,numop=final_countops(errexp,tabuassign,inter) 
    print('\n\n Numerator Final Error-bound expression: '+bestexp)
    file.write('\n\n Numerator Final Error-bound expression: '+bestexp)
    print('\nNumerator number of operations: '+str(numop))
    file.write('\nNumerator number of operations: '+str(numop))
    ...
    file.write('\n\nTabusearch steps: \n')
    #errexp=expand(errexp)
    bestexp, tabuassign, bestnumop=tabusearch(errexp,numpar,numsearch,listsize,cyclelen,secondcyclelen,file,'den')
        
    asg,numop=final_countops(errexp,tabuassign,inter) 
    print('\n\nDenominator Final Error-bound expression: '+bestexp)
    file.write('\n\nDenominator Final Error-bound expression: '+bestexp)
    print('\nDenominator Number of operations: '+str(numop))
    file.write('\nDenominator Number of operations: '+str(numop))
    ...
    outfile.write('\n*result ='+ str(inputres)+"\n")
    diverrlist=diverr(inputres,numer,denom,numerrvar,denerrvar,epval)
    writelist(diverrlist,outfile)
    condition = str(denom)+"<="+str(denerrvar)+"? 1/0:divE"
    outfile.write('\n*error_bound = ' + condition)
    outfile.close()
...
def diverr(res,num,den,numerr,denerr,epval):
    asg=[]
    resab= str(res)+'=fabs('+str(res)+')'
    divA="A = ("+str(den*numerr+num*denerr)+')*1.0000000001'
    divB="B = ("+str(den)+"-1.0000000001*"+str(denerr)+")*"+str(1-2*epval)
    s1e=str(1+2*epval)
    divE="E = (A/"+str(den)+"*"+s1e+"/B*"+s1e+"+"+str(res)+"*"+str(epval)+"*"+s1e+")*"+s1e
    asg.append(resab)
    asg.append(divA)
    asg.append(divB)
    asg.append(divE)
